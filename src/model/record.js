import {deleteRecord, getRecordList, stopRecord, recordDetail, caseDetail} from '@/services/record';
...
const recordModel = {
  ...
  effects: {
    * fetRecordList({payload}, {call, put}) {
      const resData = yield call(getRecordList, payload);
    ...
    * fetDeleteTask({payload, callback}, {call, put}) {
      const resData = yield call(deleteRecord, payload);
      callback(resData)
    },
    * fetStopTask({payload, callback}, {call, put}) {
      const resData = yield call(stopRecord, payload);
      callback(resData)
    },
