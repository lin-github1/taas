...
const PieChart: React.FC<PieProps> = (props) => {
  ...
  const modifyData = ()=>{
    ...
    console.log("1232", legendData)
    const filteredLegendData = newLegendData.filter(l => l.checked).map(l => l.x);

    // console.log(filteredLegendData)
    if (chartRef.current) {
      chartRef.current.filter('x', val => {
        console.log(val, filteredLegendData, filteredLegendData.indexOf(val + ''))
        return legendData ? filteredLegendData.indexOf(val + '') > -1: true
      });
    }
  ...
  const getLegendData = () => {
    ...
    const data = items.map((item: { color: any; _origin: any, checked: any }[]) => {
